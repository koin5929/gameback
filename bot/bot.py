...
class ReservationModal(discord.ui.Modal, title="사전예약 · 마인크래프트 닉네임 확인"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return await interaction.followup.send("⚠️ 검증 서버 연결 실패. 잠시 후 다시 시도해주세요.", ephemeral=True)

        if not v.get("ok"):
            return await interaction.followup.send("❌ 존재하지 않는 닉네임입니다. 철자/대소문자를 확인해주세요.", ephemeral=True)
        ...
        try:
            ...
            if r.status_code in (200, 201):
                ...
                return await interaction.followup.send(f"⚠️ 처리 실패({r.status_code}). 잠시 후 다시 시도해주세요.", ephemeral=True)
        except Exception:
            return await interaction.followup.send("⚠️ 서버 연결 실패. 잠시 후 다시 시도해주세요.", ephemeral=True)
...
class ReservationView(discord.ui.View):
    ...
    async def open_modal(self, interaction: discord.Interaction, button: discord.ui.Button):
        modal = ReservationModal(interaction.user)
