...
class ReservationOut(BaseModel):
    id: int; discord_id: str; nickname: str; created_at: str
...
async def resolve_uuid(name: str):
    ...
    try:
        ...
        if j and "uuid" in j: return {"uuid": j["uuid"].lower(), "name": j.get("username", name)}
    except Exception: pass
    try:
...
class ValidateOut(BaseModel):
    ok: bool; uuid: str | None = None; name: str | None = None
...
async def validate_name(name: str = Query(..., min_length=3, max_length=16)):
    ...
    if not data: return ValidateOut(ok=False)
...
async def register(payload: RegisterIn, db: Session = Depends(get_db)):
    ...
    if not resolved: raise HTTPException(status_code=422, detail="Invalid Minecraft name")
    entry = Reservation(discord_id=payload.discord_id, nickname=payload.nickname,
                        mc_uuid=resolved["uuid"], mc_name=resolved["name"])
    db.add(entry)
    try:
        db.commit(); db.refresh(entry)
    except IntegrityError:
        db.rollback(); raise HTTPException(status_code=409, detail="Already registered")
    return ReservationOut(id=entry.id, discord_id=entry.discord_id,
...
@app.get("/")
def index():
    return {"message": "Prelaunch API running"}
